...
export default class MarpSlides extends Plugin {
	...
	async showPreviewSlide() {
		...
		if (typeof replacedContent !== "string") {
			console.error("Error: replacedContent is not a string");
			return;
		}

		// this.editorView.file が null でないことを確認
		if (!this.editorView.file) {
			console.error("Error: editorView.file is null");
			return;
		}
...
export class MarpSlidesSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			.setName("Theme Path")
			.setDesc(
				"Local paths to additional theme CSS for Marp core and Marpit framework. The rule for paths is following Markdown: Styles."
			)
			.addText((text) =>
				text
		...
		new Setting(containerEl)
			.setName("Export Path")
			.setDesc(
				"Sets the custom path to export PDF, PPTX, and images. If it's empty, Marp will export in the same folder of the note. Export path does not affect HTML export"
			)
			.addText((text) =>
				text
		...
		new Setting(containerEl)
			...
			.setDesc(
				"(Experimental) Controls HTML library for eporting HTML File in Marp Cli. bespoke.js is experimental"
			)
			.addDropdown((toggle) =>
				toggle
		...
		new Setting(containerEl)
			...
			.setDesc(
				"(Experimental) Sync the slide preview with the editor cursor"
			)
			.addToggle((toggle) =>
				toggle
		...
		new Setting(containerEl)
			...
			.setDesc(
				"(Experimental) Enable the Markdown It Plugins (Mark, Containers, Kroki)"
			)
			.addToggle((toggle) =>
				toggle
...
class LineSelectionListener extends EditorSuggest<string> {
	...
	onTrigger(
		...
		let triggerInfo: EditorSuggestTriggerInfo = {
			start: cursor,
			end: cursor,
			query: "",
		};
